...
import { TarefaClass, BugTask, FeatureTask, Categoria } from '../tasks/index.js';
...
function mostrarErro(mensagem: string): void {
    const pErro = document.getElementById("pErro") as HTMLParagraphElement;
    if (pErro) {
...
export function setupUserForm(): void {
    const nomeInput = document.querySelector("#user-name") as HTMLInputElement;
    const emailInput = document.querySelector("#user-email") as HTMLInputElement;
    const adicionarBtn = document.querySelector("#user-add-btn") as HTMLButtonElement;

    if (!nomeInput || !emailInput || !adicionarBtn) return;
...
export function setupTaskForm(): void {
    ...
    const button = document.querySelector("#task-add-btn") as HTMLButtonElement;
    const categoriaSelect = document.getElementById("task-categoria") as HTMLSelectElement;
    const tipoSelect = document.getElementById("task-tipo") as HTMLSelectElement;
    const utilizadorSelect = document.getElementById("task-utilizador") as HTMLSelectElement;

    if (!input || !button || !categoriaSelect || !tipoSelect || !utilizadorSelect) return;

    button.addEventListener("click", () => {
        const taskText = input.value.trim();
        const categoriaSelecionada = categoriaSelect.value as Categoria;
